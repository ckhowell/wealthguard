...
def sync_prices():
    ...
    cursor.execute("""
        SELECT symbol, asset_name, asset_class, native_currency
        FROM holdings
        WHERE shares > 0
    ...
    for holding in holdings:
        ...
        try:
            if asset_class == 'crypto':
                ...
                # Stocks/ETFs - AUD-native tickers need the .AX suffix on yfinance.
                native = (holding['native_currency'] or '').upper()
                ticker_sym = f"{symbol}.AX" if native == 'AUD' else symbol
