...
const analyzePortfolioRisk = (holdings: RiskAnalysisHolding[]): RiskFactor[] => {
...
const RiskAnalysis = () => {
  ...
  const [holdings] = usePersistentState<RiskAnalysisHolding[]>('wealthguard-holdings', []);
  ...
  const getRiskColor = (level: string) => {
    const base = 'bg-white dark:bg-stone-900 border-stone-200 dark:border-stone-800 text-stone-900 dark:text-stone-100 border-l-2';
    switch (level) {
      case 'critical': return `${base} border-l-rose-500`;
      case 'high':     return `${base} border-l-rose-400`;
      case 'medium':   return `${base} border-l-yellow-500`;
      case 'low':      return `${base} border-l-emerald-500`;
      default:         return `${base} border-l-stone-400`;
    }
  };
  ...
  return (
    <div className="space-y-6">
      {/* Overall Risk Score */}
      <div className={`rounded-2xl p-6 border-2 ${getRiskColor(overallRisk.level)}`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 rounded-full bg-white dark:bg-stone-900 flex items-center justify-center shadow-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-stone-900 rounded-2xl shadow-sm border border-stone-200 dark:border-stone-800 p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-xs font-mono uppercase text-stone-500 dark:text-stone-400">Risk Profile Radar</h2>
          </div>
        ...
        <div className="bg-white dark:bg-stone-900 rounded-2xl shadow-sm border border-stone-200 dark:border-stone-800 p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-xs font-mono uppercase text-stone-500 dark:text-stone-400">Risk Factors ({risks.length})</h2>
          </div>
          
          <div className="space-y-3 max-h-80 overflow-y-auto">
            {risks.length === 0 ? (
              <div className="p-4 bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 border-l-2 border-l-emerald-500 rounded-lg text-center">
                <CheckCircle className="w-8 h-8 text-emerald-500 mx-auto mb-2" />
                <p className="text-stone-900 dark:text-stone-100">No significant risks detected</p>
                <p className="text-base text-stone-700 dark:text-stone-300">Your portfolio appears well-balanced</p>
              </div>
            ) : (
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-1">
                        <h3>{risk.name}</h3>
                        <span className="text-base font-bold">{risk.score}/100</span>
                      </div>
                      
                      <p className="text-base text-stone-600 dark:text-stone-300 mb-2">{risk.description}</p>
      ...
      <div className="bg-white dark:bg-stone-900 rounded-2xl shadow-sm border border-stone-200 dark:border-stone-800 p-6">
        <h2 className="text-xs font-mono uppercase text-stone-500 dark:text-stone-400 mb-4">Recommended Actions</h2>
        
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="p-4 bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 border-l-2 border-l-cyan-500 rounded-lg">
            <h3 className="text-stone-900 dark:text-stone-100 mb-2">1. Diversify Real Estate</h3>
          ...
          <div className="p-4 bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 border-l-2 border-l-cyan-500 rounded-lg">
            <h3 className="text-stone-900 dark:text-stone-100 mb-2">2. Crypto Rebalancing</h3>
            <p className="text-base text-stone-700 dark:text-stone-300">
          ...
          <div className="p-4 bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 border-l-2 border-l-cyan-500 rounded-lg">
            <h3 className="text-stone-900 dark:text-stone-100 mb-2">3. Build Equity Position</h3>
            <p className="text-base text-stone-700 dark:text-stone-300">
