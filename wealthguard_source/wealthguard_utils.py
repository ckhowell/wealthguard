...
def import_csv_transactions(file_path, account_name, date_format=None):
    ...
    return {"imported": imported_count, "skipped": skipped_count, "account_id": account_id}
...
def _live_fx(conn, from_curr: str, to_curr: str) -> float:
    """Fetch most recent rate from fx_rates, with sensible fallbacks. Used by
    update_net_worth so snapshots honour live FX the same way the API does."""
    ...
    if inv and inv[0]:
        return 1.0 / float(inv[0])
    fallback = {
        ('USD', 'AUD'): 1.55, ('AUD', 'USD'): 0.645,
        ('JPY', 'AUD'): 0.0105, ('AUD', 'JPY'): 95.0,
        ('EUR', 'AUD'): 1.65, ('GBP', 'AUD'): 1.95,
    }
...
def update_net_worth():
    """
    Record current net worth snapshot in AUD, honouring live FX rates for
    USD-denominated holdings (crypto, equity) and JPY/other cash accounts.
    Kept in lock-step with wealthguard_api._net_worth_breakdown_aud().
    """
    conn = sqlite3.connect(DB_PATH)
    conn.row_factory = sqlite3.Row
    cursor = conn.cursor()

    # Investments: holdings converted to AUD (crypto/equity are USD-priced;
    # alternative real-estate/vehicle rows are AUD-native).
    investments = 0.0
    for h in cursor.execute(
        "SELECT shares, current_price, asset_class FROM holdings WHERE shares > 0"
    ).fetchall():
        price = h["current_price"] or 0
        shares = h["shares"] or 0
        cls = (h["asset_class"] or "").lower()
        fx = _live_fx(conn, "USD", "AUD") if cls in ("crypto", "equity") else 1.0
        investments += price * shares * fx

    # Cash: savings + checking, FX'd to AUD.
    assets = 0.0
    for a in cursor.execute(
        "SELECT current_balance, currency FROM accounts "
        "WHERE is_active = 1 AND type IN ('checking', 'savings')"
    ).fetchall():
        bal = a["current_balance"] or 0
        assets += bal * _live_fx(conn, a["currency"] or "AUD", "AUD")

    total_assets = assets + investments

    # Liabilities: active credit accounts, FX'd to AUD.
    liabilities = 0.0
    for a in cursor.execute(
        "SELECT current_balance, currency FROM accounts "
        ...
        bal = a["current_balance"] or 0
        liabilities += bal * _live_fx(conn, a["currency"] or "AUD", "AUD")
